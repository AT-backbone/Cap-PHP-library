 ...
 */
	...
	class Read_CAP_Class{
		...
		function output(){
			...
			if(!empty($this->required)){
				foreach($this->required as $req){
					if(empty($this->{$req}) || (is_array($this->{$req}) && empty($this->{$req}[0]) )){
						$error['msg_format'].= $this->formError(500,"capimport.php",$req, "Process aborted, Syntax check is no longer possible", $req." is required!");
						/*
						 Fehlermeldung, im falle, dass ein Feld leer ist.
						*/
						$error['cnt_format'] ++;
					}
				}
		 ...
		 */
		function formError($Error_Num="",$Error_Process="",$Error_function="",$Error_Log="",$Error_Tipp="")
		{
			/* ============================================================================== */
			/* 									    Error style                               */
			/* ============================================================================== */
	...
	class alert extends Read_CAP_Class{
		...
		class info extends Read_CAP_Class{
			...
			function _setParameter(parameter $parameter){
				$this->parameter[$this->parameterIndex] = $parameter;
				$this->parameterIndex++;
			}
			
			function _setResource(resource $resource){
				$this->resource[$this->resourceIndex] = $resource;
				$this->resourceIndex++;
			}
			
			function _setArea(area $area){
		...
		class area extends Read_CAP_Class{
			...
			protected $subclass = array('geocode'=>'parameter');
			var   $areaDesc;
			var   $polygon = array();
			...
			function _setGeocode(parameter $geocode){
		...
		class resource extends Read_CAP_Class{
		...
		class parameter extends Read_CAP_Class{
