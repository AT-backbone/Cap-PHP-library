		...
		foreach($files2 as $file) {
			if($file != "." && $file != ".." && $file != "ftp_prc" && $file != "skeleton_green" && $file != "COPYING") {
				...
				if ($_POST[filename]) {
					...
					$authentication_2=array(
						'dolibarrkey'=> $configuration->conf["webservice"]["securitykey_2"],
						'sourceapplication'=> $configuration->conf["webservice"]["WS_METHOD_2"],
						'login'=>  $configuration->conf["webservice"]["login_2"],
						'password'=> $configuration->conf["webservice"]["password_2"]);
                    ...
                    $document_2=array(
                        'filename'=> $_POST[filename],
                        'mimetype'=> 'text/xml',
                        'content'=> base64_encode($contents),						//$_FILES["uploadfile"]["tmp_name"]
					...
					$parameters_2 = array('authentication'=>$authentication_2, 'document'=>$document_2);
					...
					$out.= '<h2>Response from<br>'.$configuration->conf['webservice']['WS_DOL_URL'].'</h2>';
					$out.= '<h3>Message:</h3>';
					...
					if($soapclient_2 != '') {
						$result_2 = $soapclient_2->call($configuration->conf["webservice"]["WS_METHOD_2"],$parameters_2,$configuration->conf["webservice"]["ns_2"],'');
						$out .= '<br><hr><br>';					

						if($result_2["syntaxcheck"]["capformat_error"]==1) 	$format_error = "YES"; else $format_error = "NO";
						if($result_2["syntaxcheck"]["capvalue_error"]==1) 	$value_error = "YES"; else $value_error = "NO";
						if($result_2["syntaxcheck"]["cnt_errors"]>0) 		$error_bool = '<p>CAP format Error:'.$format_error.'<p>CAP value Error:'.$value_error.'<p>Cnt Errors:'.$result["syntaxcheck"]["cnt_errors"].'<p>';

						$out.= '<h2>Response from<br>'.$configuration->conf['webservice']['WS_DOL_URL_2'].'</h2>';
						$out.= '<h3>Message:</h3>';
						$out.= '<pre>'.$result_2["result"]["result_label"].'</pre>';
						$out.= '<pre>'.$result_2["result_label"].'</pre>';
						$out.= '<pre>'.$error_bool.$result_2["syntaxcheck"]["error_log"].$result_2["syntaxcheck"]["debug_msg"].'</pre>';

						if ($soapclient_2->fault) {
							$out.= '<h2>Fault SoapClient 2</h2><pre>';
							$out.=var_dump($result_2);
							$out.= '</pre>';
						} else {
							// Check for errors
							$err = $soapclient_2->getError();
							if ($err) {
								// Display the error
								$out.= '<h2>Error </h2><pre>' . $err . '</pre>';
							} else {
