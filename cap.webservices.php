...
if ($_POST[filename])
{
	...
	$soapclient = new nusoap_client($conf->WS_DOL_URL); // <-- set the Timeout above 300 Sec.
	if ($soapclient)
	{
	...
	$authentication=array(
	    'dolibarrkey'=>$conf->dolibarrkey,
	    'sourceapplication'=>$conf->WS_METHOD,
	  	'login'=> $conf->login,
  	  'password'=> $conf->password);
 
  $tmpfile = $conf->destination.'/'.$_POST[filename];
	...
	$document=array(
		...
		$result = $soapclient->call($conf->WS_METHOD,$parameters,$conf->ns,'');
		if ($soapclient->fault) {
